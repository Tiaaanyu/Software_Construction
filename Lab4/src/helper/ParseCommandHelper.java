...
public class ParseCommandHelper {
	...
	public static void parseAndExecuteCommand(String string, Graph<Vertex, Edge> graph) {
		...
		if(addVertexMatcher.matches()) {
			...
			log.Log.logger.info("Addvertex, " + label + ", " + type + ", " + graph);
		}
		...
		if(deleteVertexMatcher.matches()) {
			...
			log.Log.logger.info("Delete vertex, " + deleteVertexMatcher.group(1));
		}
		...
		if(addEdgeMatcher.matches()) {
			...
			log.Log.logger.info("Addedge, " + label + ", " + graph);
		}
		...
		if(deleteEdgeMatcher.matches()) {
			...
			log.Log.logger.info("DeleteEdge, " + deleteEdgeMatcher.group(1));
		}
		...
		if(addHyperEdgeMatcher.matches()) {
			...
			graph.addEdge(edge);
			log.Log.logger.info("AddHyperEdge, " + addHyperEdgeMatcher.group(1));
		}
	}
